	...
	public void processBindingConfiguration(String context, Item item, String bindingConfig) throws BindingConfigParseException {		
		...
		try{
			...
			if (properties.length > 2)
				config.minAngle = Float.parseFloat(properties[2]);
			else
				config.minAngle = PCA9685PwmControl.MIN_ANGLE_DEFAULT;
			
			if (properties.length > 3)
				config.maxAngle = Float.parseFloat(properties[3]);
			else
				config.maxAngle = PCA9685PwmControl.MAX_ANGLE_DEFAULT;
			
			if (properties.length > 4)
				config.degreeMilliseconds = Integer.parseInt(properties[4]);
			else
				config.degreeMilliseconds = PCA9685PwmControl.DEGREE_MILLISECONDS;
			
			checkOfValidValues(config, item.getName());
		}catch(Exception e){
			logger.error("Exception in processBindingConfiguration.", e);
		}

		logger.debug("pca9685controlGenericBindingProvider: processBindingConfiguration({},{}) is called!",
				config.address, config.pinNumber);
		...
		try {
			if(!PCA9685Map.containsKey(config.address)){
				try{
					...
					logger.error("Exception in handleBoards.", e);
				}				
			}
